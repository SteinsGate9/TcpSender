...
void single_send(cmu_socket_t * sock, char* data, int buf_len){
    ...
    if(buf_len > 0){
      while(buf_len != 0){
        seq = sock->window.last_ack_received; /* c */

        if(buf_len <= MAX_DLEN){ /* MAX_DLEN is max send once  */
            plen = DEFAULT_HEADER_LEN + buf_len; /* send headerlen + buf_len */

            msg = create_packet_buf(sock->my_port, sock->their_port, seq, seq,  /* create buf */
...
void* begin_backend(void * in){
  ...
  while(TRUE){
    ...
    if(buf_len > 0){ /* if have stuff to send, send them all */
      data = malloc(buf_len);
